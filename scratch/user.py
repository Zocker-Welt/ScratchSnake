...
class User:
    ...
    def follower_count(self):
        return int(re.search(r'Followers \(([0-9]+)\)', req.get(f"https://scratch.mit.edu/users/{self.username}/followers").text, re.I).group(1))
    
    def following_count(self):
        return int(re.search(r'Following \(([0-9]+)\)', req.get(f"https://scratch.mit.edu/users/{self.username}/following/").text, re.I).group(1))